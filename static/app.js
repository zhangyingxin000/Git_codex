...
function tableCases(){if(!data.cases.length)return '<div class="empty"><b>暂无测试用例</b>AI 生成后将在此处管理</div>';let labels={NOT_RUN:'未执行',PASSED:'通过',FAILED:'失败',BLOCKED:'阻塞',ERROR:'环境异常',ASSERTION_PASSED:'原子断言通过'},counts={};data.cases.forEach(x=>counts[x.execution_status||'NOT_RUN']=(counts[x.execution_status||'NOT_RUN']||0)+1);return `<div class="card"><h3>需求驱动用例集</h3><p style="font-size:12px;color:var(--muted)">共 ${data.cases.length} 条；真实接口、UI真机、数据库/Redis、性能和安全由各自执行器取证。Mock 与未执行项不计真实通过率。</p><p>${Object.entries(counts).map(([k,v])=>`<span class="tag ${k}">${labels[k]||k} ${v}</span>`).join(' ')}</p><div class="formrow"><input placeholder="搜索标题、场景、需求依据或执行器" oninput="filterRows('caseRows',this.value,'')"><select onchange="filterRows('caseRows','',this.value)"><option value="">全部执行器</option><option value="http">真实HTTP</option><option value="workflow">关联流程</option><option value="api_db">接口+数据库</option><option value="db_redis">数据库+Redis</option><option value="ui_device">UI真机</option><option value="api_ui">接口+UI</option><option value="performance_ui">性能+UI</option><option value="api_security">安全接口</option></select></div></div><table><thead><tr><th>优先级</th><th>用例 / 需求依据</th><th>场景 / 执行器</th><th>预期结果</th><th>实际结果</th><th>执行状态</th><th></th></tr></thead><tbody id="caseRows">${data.cases.map(x=>{let st=x.execution_status||'NOT_RUN';return `<tr data-risk="${esc(x.executor_type||'manual')}" data-search="${esc((x.title+' '+x.scenario_type+' '+x.executor_type+' '+x.requirement_ref).toLowerCase())}"><td><span class="tag ${x.priority}">${x.priority}</span></td><td><b>${esc(x.title)}</b><br><small>${esc(x.requirement_ref||'未标注')}</small></td><td><span class="tag">${esc(x.scenario_type||'-')}</span><br><small>${esc(x.executor_type||'manual')}</small></td><td>${esc(x.expected||'未填写')}</td><td>${esc(x.actual_result||'尚无真实执行证据')}</td><td><span class="tag ${st}">${labels[st]||st}</span></td><td>${x.method?`<button class="small" onclick="runCase('${x.id}')">执行真实请求</button>`:st==='BLOCKED'?'等待条件':'待对应执行器'}</td></tr>`}).join('')}</tbody></table>`}
...
function applyPerformanceProfile(){let p=$('#toolPerfProfile')?.value||'smoke',m={smoke:{threads:2,loops:5,rampup:2,timeout:180,error:0,p95:3000,p99:5000,tps:0},baseline:{threads:10,loops:20,rampup:30,timeout:600,error:1,p95:1500,p99:2500,tps:2},load:{threads:30,loops:30,rampup:60,timeout:900,error:1,p95:2000,p99:3500,tps:5},stability:{threads:20,loops:100,rampup:60,timeout:1800,error:.5,p95:2000,p99:4000,tps:2}}[p]||{};[['#toolJmeterThreads','threads'],['#toolJmeterLoops','loops'],['#toolJmeterRampup','rampup'],['#toolJmeterTimeout','timeout'],['#toolMaxErrorRate','error'],['#toolMaxP95','p95'],['#toolMaxP99','p99'],['#toolMinThroughput','tps']].forEach(([id,key])=>{let el=$(id);if(el&&m[key]!==undefined)el.value=m[key]})}
...
const openProjectBase=openProject;openProject=async function(id){await openProjectBase(id);try{data.consistency=await api(`/api/projects/${id}/consistency`)}catch{data.consistency={rules:[],runs:[]}}try{data.wealth_latest=await api(`/api/projects/${id}/wealth-latest-report`);if(!wealthLastResult||!Object.keys(wealthLastResult).length)wealthLastResult=data.wealth_latest}catch{data.wealth_latest={}}try{data.gift_latest=await api(`/api/projects/${id}/gift-latest-report`)}catch{data.gift_latest={}}try{data.evidence_center=await api(`/api/projects/${id}/evidence-center`)}catch{data.evidence_center={}}try{data.generated_reports=await api(`/api/projects/${id}/reports`)}catch{data.generated_reports=[]}try{data.test_accounts=await api(`/api/projects/${id}/test-accounts`)}catch{data.test_accounts=[]}try{data.quality_profile=await api(`/api/projects/${id}/quality-profile`)}catch{data.quality_profile={sections:[],score:0}}try{data.diagnosis=await api(`/api/projects/${id}/diagnosis`)}catch{data.diagnosis={items:[],summary:{}}}render()};
...
function performanceReportListHtml(){let perf=(data.generated_reports||[]).filter(x=>x.kind==='JMeter');if(!perf.length)return '<div class="empty"><b>暂无JMeter性能报告</b></div>';return `<div class="card">${perf.map(x=>`<div class="summary-panel"><b>${esc(x.name)}</b><p>${esc(x.summary)}</p>${x.performance_diagnosis?performanceDiagnosisHtml(x.performance_diagnosis,x.performance_summary||{}):''}${x.html_url?`<button class="small" onclick="openReport('${esc(x.html_url)}')">查看JMeter HTML</button> `:''}${x.json_url?`<button class="small" onclick="openReport('${esc(x.json_url)}')">查看JSON</button>`:''}</div>`).join('')}</div>`}
...
professionalizeWorkspace=function(){
  professionalLabels();
  if($('#overview'))$('#overview').innerHTML=tableQualityHub();
...
function resourceManifestLines(items,type){
  ...
  if(type==='runtime')return (items||[]).map(x=>typeof x==='string'?x:[x.name||'',x.purpose||''].join('|')).join('\n');
...
async function saveRequirementResourceManifest(packageId){
  try{
    let payload={
      ...
      runtime_parameters:parseResourceLines('resourceRuntimeParameters',x=>({name:x[0]||'',purpose:x[1]||''})),
