...
OPENAPI_TAGS = [
    ...
    {
        "name": "Assets",
        "description": "需求资料、接口资产、测试点、测试用例和追踪关系。",
    },
    {
        "name": "Execution",
        "description": "接口自动化、业务流程、性能、安全和调度执行。",
    },
    {
        "name": "Data Validation",
        "description": "MySQL/Redis 只读映射、快照和一致性规则。",
    },
...
def create_app() -> FastAPI:
    ...
    def http_exception_handler(_: Request, exc: HTTPException) -> JSONResponse:
        ...
        return JSONResponse(
            ...
            content={"error": message, "detail": exc.detail},
        )
    ...
    def value_error_handler(_: Request, exc: ValueError) -> JSONResponse:
        return JSONResponse(
            status_code=400, content={"error": str(exc), "detail": str(exc)}
        )
    ...
    @api.get(
        "/api/system/storage-policy", tags=["System"], summary="查看平台存储与只读边界"
    )
    def storage_policy() -> dict[str, Any]:
        return legacy.platform_storage_policy()

    @api.get(
        "/api/system/environment-config",
        tags=["System"],
        summary="查看YAML环境配置状态",
    )
    def environment_config() -> dict[str, Any]:
        return legacy.environment_config_status()

    @api.get(
        "/api/projects/{project_id}/environment-config",
        tags=["Projects"],
        summary="项目YAML环境配置状态",
    )
    ...
    @api.get(
        "/api/projects/{project_id}/diagnosis",
        tags=["Projects"],
        summary="平台自动缺口诊断",
    )
    ...
    @api.get(
        "/api/projects/{project_id}/control-plane",
        tags=["Projects"],
        summary="AI 自动化质量中枢控制面",
    )
    ...
    def create_requirement_package(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
    ...
    def generate_requirement_account_model(
    ...
    def save_resource_manifest(
        ...
        return legacy.save_requirement_resource_manifest(
            project_id, package_id, payload
        )
    ...
    @api.get(
        "/api/projects/{project_id}/requirement-packages/{package_id}/execution-plan",
        tags=["Execution"],
        summary="需求包场景计划",
    )
    ...
    def generate_requirement_execution_plan(
        project_id: str,
        package_id: str,
        payload: dict[str, Any] = Body(default_factory=dict),
    ) -> dict[str, Any]:
        return legacy.generate_requirement_execution_plan(
            project_id, package_id, payload
        )

    @api.post(
        "/api/projects/{project_id}/requirement-packages/{package_id}/tool-assets",
        tags=["Execution"],
        summary="生成需求包工具资产",
    )
    def requirement_tool_assets(
        project_id: str,
        package_id: str,
        payload: dict[str, Any] = Body(default_factory=dict),
    ) -> dict[str, Any]:
        return legacy.generate_requirement_package_tool_assets(
            project_id, package_id, payload
        )

    @api.post(
        "/api/projects/{project_id}/requirement-packages/{package_id}/runs",
        tags=["Execution"],
        summary="创建需求包统一运行批次",
    )
    def create_requirement_run(
        project_id: str,
        package_id: str,
        payload: dict[str, Any] = Body(default_factory=dict),
    ) -> dict[str, Any]:
    ...
    def requirement_run(
        project_id: str, package_id: str, run_id: str
    ) -> dict[str, Any]:
        return legacy.get_requirement_run_context(project_id, package_id, run_id)

    @api.post(
        "/api/projects/{project_id}/requirement-packages/{package_id}/newman/run",
        tags=["Execution"],
        summary="运行需求包Newman",
    )
    def requirement_newman_run(
        project_id: str,
        package_id: str,
        payload: dict[str, Any] = Body(default_factory=dict),
    ) -> dict[str, Any]:
    ...
    @api.post(
        "/api/projects/{project_id}/requirement-packages/{package_id}/pytest/run",
        tags=["Execution"],
        summary="运行需求包pytest证据复核",
    )
    def requirement_pytest_run(
        project_id: str,
        package_id: str,
        payload: dict[str, Any] = Body(default_factory=dict),
    ) -> dict[str, Any]:
    ...
    @api.post(
        "/api/projects/{project_id}/salary-trade/jmeter-harvest",
        tags=["Execution"],
        summary="回收工资交易JMeter执行报告",
    )
    def salary_trade_jmeter_harvest(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
    ...
    def requirement_scenario_report(
        project_id: str,
        package_id: str,
        payload: dict[str, Any] = Body(default_factory=dict),
    ) -> dict[str, Any]:
        return legacy.generate_requirement_package_unified_scenario_report(
            project_id, package_id, payload
        )
    ...
    @api.post(
        "/api/projects/{project_id}/requirement-packages/{package_id}/ai-review",
        tags=["Reports"],
        summary="生成需求包AI复盘",
    )
    def requirement_ai_review(
        project_id: str,
        package_id: str,
        payload: dict[str, Any] = Body(default_factory=dict),
    ) -> dict[str, Any]:
        return legacy.generate_requirement_package_ai_review(
            project_id, package_id, payload
        )

    @api.post(
        "/api/projects/{project_id}/requirement-packages/{package_id}/performance-ai-review",
        tags=["Reports"],
        summary="生成当前批次性能报告AI分析",
    )
    def requirement_performance_ai_review(
        project_id: str,
        package_id: str,
        payload: dict[str, Any] = Body(default_factory=dict),
    ) -> dict[str, Any]:
        return legacy.generate_requirement_performance_ai_review(
            project_id, package_id, payload
        )

    @api.post(
        "/api/projects/{project_id}/requirement-packages/{package_id}/newman-analysis",
        tags=["Reports"],
        summary="生成当前批次Newman接口冒烟分析",
    )
    def requirement_newman_analysis(
        project_id: str,
        package_id: str,
        payload: dict[str, Any] = Body(default_factory=dict),
    ) -> dict[str, Any]:
        return legacy.generate_requirement_newman_analysis(
            project_id, package_id, payload
        )

    @api.post(
        "/api/projects/{project_id}/requirement-packages/{package_id}/jmeter-load-plan",
        tags=["Execution"],
        summary="生成可执行JMeter持续压测预案",
    )
    def requirement_jmeter_load_plan(
        project_id: str,
        package_id: str,
        payload: dict[str, Any] = Body(default_factory=dict),
    ) -> dict[str, Any]:
        return legacy.generate_requirement_jmeter_load_plan(
            project_id, package_id, payload
        )

    @api.post(
        "/api/projects/{project_id}/requirement-packages/{package_id}/jmeter-load-run",
        tags=["Execution"],
        summary="执行JMeter持续压测阶梯并回收JTL与HTML",
    )
    def requirement_jmeter_load_run(
        project_id: str,
        package_id: str,
        payload: dict[str, Any] = Body(default_factory=dict),
    ) -> dict[str, Any]:
    ...
    @api.post(
        "/api/projects/{project_id}/structured-test-cases",
        tags=["Assets"],
        summary="生成结构化测试用例",
    )
    def structured_test_cases(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
        return legacy.generate_structured_test_cases(project_id, payload)

    @api.post(
        "/api/projects/{project_id}/candidate-evidence-rules",
        tags=["Data Validation"],
        summary="生成候选证据规则",
    )
    def candidate_evidence_rules(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
        return legacy.generate_candidate_evidence_rules(project_id, payload)

    @api.post(
        "/api/projects/{project_id}/accept-candidate-evidence-rules",
        tags=["Data Validation"],
        summary="采纳候选证据规则",
    )
    def accept_candidate_evidence_rules(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
        return legacy.accept_candidate_evidence_rules(project_id, payload)

    @api.post(
        "/api/projects/{project_id}/business-evidence-plan",
        tags=["Data Validation"],
        summary="生成业务证据计划",
    )
    def business_evidence_plan(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
        return legacy.generate_business_evidence_plan(project_id, payload)

    @api.post(
        "/api/projects/{project_id}/business-evidence-run",
        tags=["Data Validation"],
        summary="执行业务证据规则",
    )
    def business_evidence_run(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
        return legacy.run_business_evidence_rules(project_id, payload)

    @api.post(
        "/api/projects/{project_id}/metadata-hallucination-audit",
        tags=["Data Validation"],
        summary="AI输出元数据静态校验",
    )
    def metadata_hallucination_audit(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
        return legacy.metadata_hallucination_audit(project_id, payload)

    @api.post(
        "/api/projects/{project_id}/metadata-hallucination-correction",
        tags=["Data Validation"],
        summary="生成一次元数据修正版",
    )
    def metadata_hallucination_correction(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
        return legacy.metadata_hallucination_correction(project_id, payload)

    @api.get(
        "/api/projects/{project_id}/quality-profile",
        tags=["Projects"],
        summary="项目配置画像",
    )
    ...
    @api.get(
        "/api/projects/{project_id}/reports", tags=["Reports"], summary="项目报告列表"
    )
    def reports(project_id: str) -> list[dict[str, Any]]:
        return legacy.list_generated_reports(project_id)

    @api.get(
        "/api/projects/{project_id}/test-accounts",
        tags=["Execution"],
        summary="测试账号池",
    )
    ...
    @api.get(
        "/api/projects/{project_id}/evidence-center",
        tags=["Reports"],
        summary="证据中心状态",
    )
    ...
    @api.get(
        "/api/projects/{project_id}/capture-reports",
        tags=["Assets"],
        summary="资料采集完整性报告",
    )
    ...
    @api.get(
        "/api/projects/{project_id}/interface-document",
        tags=["Assets"],
        summary="导出脱敏接口文档基线",
    )
    ...
    @api.post(
        ...
        tags=["Assets"],
        summary="导入Apifox OpenAPI并生成pytest基础层",
    )
    def apifox_openapi_import(
        project_id: str,
        package_id: str,
        payload: dict[str, Any] = Body(default_factory=dict),
    ) -> dict[str, Any]:
        try:
            return legacy.import_apifox_openapi_to_package(
                project_id, package_id, payload
            )
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
    ...
    def apifox_cli_run(
        ...
        try:
            return legacy.run_requirement_package_apifox_cli(
                project_id, package_id, payload
            )
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
    ...
    @api.post(
        "/api/projects/{project_id}/delivery-package",
        tags=["Assets"],
        summary="生成测试资产交付包",
    )
    ...
    @api.get(
        "/api/projects/{project_id}/toolchain",
        tags=["Execution"],
        summary="企业测试工具链预检",
    )
    ...
    @api.post(
        "/api/projects/{project_id}/tool-assets",
        tags=["Execution"],
        summary="生成企业测试工具资产",
    )
    ...
    @api.post(
        "/api/projects/{project_id}/toolchain/run",
        tags=["Execution"],
        summary="执行企业测试工具链",
    )
    def run_toolchain(
    ...
    @api.post(
        "/api/projects/{project_id}/jmeter/open-gui",
        tags=["Execution"],
        summary="打开 JMeter GUI 并加载线程组",
    )
    def open_jmeter_gui(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
    ...
    @api.post(
        "/api/projects/{project_id}/jmeter/harvest-gui-report",
        tags=["Execution"],
        summary="回收 JMeter GUI 执行报告",
    )
    def harvest_jmeter_gui_report(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
        try:
    ...
    @api.get(
        "/api/projects/{project_id}/jmeter/case-script-model",
        tags=["Execution"],
        summary="查看测试用例到 JMeter 脚本模型",
    )
    ...
    @api.post(
        "/api/projects/{project_id}/jmeter/generate-from-cases",
        tags=["Execution"],
        summary="按测试用例生成 JMeter 脚本",
    )
    ...
    @api.get(
        "/api/projects/{project_id}/execution-profile",
        tags=["Execution"],
        summary="项目运行参数与执行策略",
    )
    ...
    @api.get(
        "/api/projects/{project_id}/redis-mappings",
        tags=["Data Validation"],
        summary="接口到 Redis 映射",
    )
    ...
    @api.get(
        "/api/projects/{project_id}/consistency",
        tags=["Data Validation"],
        summary="数据一致性规则与执行记录",
    )
    ...
    @api.get(
        "/api/projects/{project_id}/wealth-latest-report",
        tags=["Reports"],
        summary="最新财富等级综合报告",
    )
    def wealth_latest_report(project_id: str) -> dict[str, Any]:
        ...
        payload["functional"] = (
            wealth_api.get("http_status") == 200
            and wealth_api.get("business_code") == 200
        )
    ...
    @api.get(
        "/api/projects/{project_id}/gift-latest-report",
        tags=["Reports"],
        summary="最新送礼链路报告",
    )
    ...
    @api.get(
        "/api/projects/{project_id}/wealth-reward-configs",
        tags=["Data Validation"],
        summary="财富奖励配置核对数据",
    )
    ...
    @api.post(
        "/api/projects/{project_id}/redis-manual-result",
        tags=["Data Validation"],
        summary="补充人工 Redis 核对结果",
    )
    ...
    @api.post(
        ...
        status_code=201,
        tags=["Assets"],
        summary="导入需求、文档、图片、OpenAPI 或 HAR",
    )
    def add_source(project_id: str, payload: SourceCreateRequest) -> dict[str, Any]:
        ...
        if payload.get("browser_capture") and payload.get("source_url"):
            content, browser_images, page_meta = legacy.browser_capture_source(
                payload["source_url"]
            )
        ...
        if all_images:
            ...
            for index, (image_name, mime, data_b64) in enumerate(all_images):
                asset_analysis = (
                    analysis
                    if index == 0
                    else "已纳入同一份需求的图文联合分析，综合结论见首张资源"
                )
                image_values.append(
                    (
                        legacy.uid("asset"),
        ...
        legacy.execute(
            ...
            (
                ...
                payload.get(
                    "name",
                    payload.get("file_name") or payload.get("source_url") or "资料",
                ),
                kind,
                content,
                legacy.now(),
        ...
        if page_meta is not None:
            score, capture_status, capture_blockers = (
                legacy.evaluate_capture_completeness(page_meta)
            )
            legacy.execute(
        ...
        if image_values:
            with legacy.db() as conn:
                conn.executemany(
                    "INSERT INTO source_assets VALUES (?,?,?,?,?,?,?,?,?)", image_values
                )
        result = (
    ...
    def generate_assets(
        project_id: str, payload: GenerateFromSourceRequest
    ) -> dict[str, Any]:
        return legacy.generate_assets(project_id, payload.source_id, payload.force)

    @api.post(
        "/api/projects/{project_id}/db-schema",
        tags=["Data Validation"],
        summary="导入 MySQL Schema 元数据",
    )
    def db_schema(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
    ...
    @api.get(
        "/api/projects/{project_id}/mysql/status",
        tags=["Data Validation"],
        summary="MySQL只读连接状态",
    )
    ...
    @api.post(
        "/api/projects/{project_id}/mysql/test",
        tags=["Data Validation"],
        summary="测试MySQL只读连接",
    )
    ...
    @api.post(
        "/api/projects/{project_id}/mysql/query",
        tags=["Data Validation"],
        summary="执行MySQL只读查询",
    )
    def mysql_query(
    ...
    @api.post(
        "/api/projects/{project_id}/mysql/import-schema-live",
        tags=["Data Validation"],
        summary="从MySQL实时读取Schema元数据",
    )
    def mysql_import_schema_live(project_id: str) -> dict[str, Any]:
        return legacy.import_mysql_schema_live(project_id)

    @api.post(
        "/api/projects/{project_id}/salary-trade/db-evidence",
        tags=["Data Validation"],
        summary="工资交易MySQL只读证据核查",
    )
    def salary_trade_db_evidence(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
        return legacy.salary_trade_db_evidence_check(project_id, payload)

    @api.post(
        "/api/projects/{project_id}/redis-sources",
        status_code=201,
        tags=["Data Validation"],
        summary="接入 Redis 只读数据源",
    )
    def add_redis_source(
        project_id: str, payload: RedisSourceCreateRequest
    ) -> dict[str, Any]:
        payload = payload.model_dump()
        ...
        return {
            **legacy.row("SELECT * FROM redis_sources WHERE id=?", (redis_id,)),
            **result,
        }

    @api.post(
        "/api/projects/{project_id}/auto-match-data",
        tags=["Data Validation"],
        summary="自动匹配接口、MySQL 与 Redis",
    )
    def auto_match_data(project_id: str) -> dict[str, Any]:
        return legacy.auto_match_project_data(project_id)

    @api.post(
        "/api/projects/{project_id}/generate-consistency",
        tags=["Data Validation"],
        summary="生成一致性验证规则",
    )
    ...
    @api.post(
        "/api/projects/{project_id}/consistency/run-ready",
        tags=["Data Validation"],
        summary="运行已补齐的一致性规则",
    )
    def run_ready_consistency(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
        return legacy.run_ready_consistency_rules(
            project_id, payload.get("limit", 5), payload.get("scope", "core")
        )

    @api.post(
        "/api/projects/{project_id}/evidence-check",
        tags=["Data Validation"],
        summary="发起一次按需数据证据核查",
    )
    def manual_evidence_check(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
        return legacy.run_manual_evidence_check(project_id, payload)

    @api.post(
        "/api/projects/{project_id}/assistant", tags=["Assets"], summary="平台助手对话"
    )
    def assistant(
        project_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, Any]:
    ...
    @api.post(
        "/api/consistency-rules/{rule_id}/run",
        tags=["Data Validation"],
        summary="执行单条一致性规则",
    )
    ...
    @api.post(
        "/api/redis-sources/{source_id}/test",
        tags=["Data Validation"],
        summary="测试 Redis 只读连接",
    )
    def test_redis_source(source_id: str) -> dict[str, Any]:
        return legacy.check_redis_source(source_id)

    @api.post(
        "/api/redis-sources/{source_id}/scan",
        tags=["Data Validation"],
        summary="扫描 Redis Key",
    )
    ...
    @api.post(
        "/api/redis-sources/{source_id}/inspect",
        tags=["Data Validation"],
        summary="查看 Redis Key 只读快照",
    )
    ...
    @api.post(
        "/api/projects/{project_id}/generate-workflows",
        tags=["Assets"],
        summary="生成业务流程",
    )
    ...
    @api.post(
        "/api/projects/{project_id}/refresh-trace",
        tags=["Assets"],
        summary="刷新需求到接口追踪关系",
    )
    ...
    @api.post(
        "/api/projects/{project_id}/ai-pipeline",
        tags=["Execution"],
        summary="运行 AI 质量流水线",
    )
    def ai_pipeline(
    ...
    @api.post(
        "/api/requirements/{requirement_id}/link",
        tags=["Assets"],
        summary="设置需求与接口关联",
    )
    ...
    @api.post(
        "/api/requirements/{requirement_id}/generate-workflow",
        tags=["Assets"],
        summary="按需求生成业务流程",
    )
    ...
    @api.post(
        "/api/cases/{case_id}/run", tags=["Execution"], summary="执行单条测试用例"
    )
    def run_case(case_id: str) -> dict[str, Any]:
        return legacy.execute_case(case_id)

    @api.post(
        "/api/projects/{project_id}/gift-chain",
        tags=["Execution"],
        summary="执行送礼业务链路",
    )
    def gift_chain(
    ...
    @api.post(
        "/api/projects/{project_id}/login-performance",
        tags=["Execution"],
        summary="执行多账号登录性能专项",
    )
    ...
    def run_all(
        project_id: str,
        payload: ProjectRuntimeRequest = Body(default_factory=ProjectRuntimeRequest),
    ) -> dict[str, Any]:
        payload = payload.model_dump()
        ...
        if primary:
            ...
            total = legacy.row(
                "SELECT COUNT(*) n FROM test_cases WHERE project_id=?", (project_id,)
            )["n"]
    ...
    def update_endpoint(
        endpoint_id: str, payload: dict[str, Any] = Body(default_factory=dict)
    ) -> dict[str, bool]:
    ...
    @api.put(
        "/api/requirements/{requirement_id}", tags=["Assets"], summary="更新需求条目"
    )
    def update_requirement(
        ...
        old = legacy.row(
            "SELECT * FROM requirement_items WHERE id=?", (requirement_id,)
        )
        if not old:
    ...
    if requirement_reports_dir.exists():
        api.mount(
            "/requirement-reports",
            StaticFiles(directory=requirement_reports_dir),
            name="requirement-reports",
        )
