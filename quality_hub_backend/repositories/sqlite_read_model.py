...
class QualityReadModelRepository:
    ...
    def project_settings(self, project_id: str) -> dict[str, str]:
        ...
        return {
            ...
            for item in self.rows("SELECT key,value FROM settings WHERE key LIKE ?", (prefix + "%",))
        }

    def requirements(self, project_id: str) -> list[dict[str, Any]]:
        return self.rows("SELECT * FROM requirement_items WHERE project_id=?", (project_id,))
    ...
    def endpoints(self, project_id: str) -> list[dict[str, Any]]:
        return self.rows("SELECT * FROM api_endpoints WHERE project_id=? ORDER BY path", (project_id,))

    def test_cases(self, project_id: str) -> list[dict[str, Any]]:
        return self.rows("SELECT * FROM test_cases WHERE project_id=? ORDER BY created_at DESC", (project_id,))

    def consistency_rules(self, project_id: str) -> list[dict[str, Any]]:
        return self.rows("SELECT * FROM consistency_rules WHERE project_id=?", (project_id,))
    ...
    def db_tables(self, project_id: str) -> list[dict[str, Any]]:
        return self.rows("SELECT table_name FROM db_tables WHERE project_id=?", (project_id,))

    def db_mappings(self, project_id: str) -> list[dict[str, Any]]:
        return self.rows("SELECT * FROM api_db_mappings WHERE project_id=?", (project_id,))

    def redis_sources(self, project_id: str) -> list[dict[str, Any]]:
        return self.rows("SELECT * FROM redis_sources WHERE project_id=?", (project_id,))

    def redis_mappings(self, project_id: str) -> list[dict[str, Any]]:
        return self.rows("SELECT * FROM api_redis_mappings WHERE project_id=?", (project_id,))
    ...
    def accounts(self, project_id: str) -> list[dict[str, Any]]:
        ...
        for account in accounts:
            secret = self._credential_loader(account["id"]) if self._credential_loader else {}
            if not secret:
                credential = self._data_dir / f"test-account-{account['id']}.bin"
                secret = {"ticket": "", "encrypted_password": ""} if not credential.is_file() else secret
    ...
    def save_diagnosis(self, project_id: str, generated_at: str, items: list[dict[str, Any]]) -> None:
        values = [
            (
                item["id"],
                project_id,
        ...
        with self._connection_factory() as conn:
            conn.execute("DELETE FROM quality_diagnosis WHERE project_id=?", (project_id,))
            conn.executemany("INSERT INTO quality_diagnosis VALUES (?,?,?,?,?,?,?,?,?,?,?)", values)
