...
def build(source_path, output_dir):
    ...
    for level in BOUNDARIES:
        ...
        payload["_mockMeta"] = {
            "mock": True,
            "scenario": f"level-{level}",
            "basis": "real-response-structure",
            "experienceValues": "synthetic-not-business-config",
        }
    ...
    abnormal = {
        ...
        "duplicate-level-range": lambda x: x["data"]["experRights"].append(
            copy.deepcopy(x["data"]["experRights"][-1])
        ),
        "invalid-resource-url": lambda x: x["data"]["experRights"][0]["right2"][
            0
        ].update({"rightImageUrl": "invalid-url"}),
        "missing-arabic-name": lambda x: x["data"]["experRights"][0]["right2"][
            0
        ].update({"rightArName": ""}),
    }
    ...
    unauthorized = {
        ...
        "data": None,
        "_mockMeta": {"mock": True, "scenario": "unauthorized"},
    }
    (output / "unauthorized.json").write_text(
        json.dumps(unauthorized, ensure_ascii=False, indent=2), encoding="utf-8"
    )
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        raise SystemExit(
            "usage: mock_wealth_responses.py REAL_RESPONSE_JSON OUTPUT_DIR"
        )
    print(
        json.dumps(
            {"generated": len(build(sys.argv[1], sys.argv[2]))}, ensure_ascii=False
        )
    )
